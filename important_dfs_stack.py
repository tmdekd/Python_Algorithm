...
for t in range(1, T + 1):
    ...
    # 노드 번호가 1부터 시작하기 때문에 v+1임. visited[0]의 경우 사용되지 않고 비워져 있음
    # visited가 1부터 시작하게 하면 인덱스가 맞지 않아. 그냥 비워둠
    for _ in range(v + 1):
        visited.append(False)  # 초기에는 모든 노드를 방문하지 않은 상태로 설정

    found = 0  # 경로 존재 여부 (0: 없음, 1: 있음)

    while stack:  # 스택이 비어 있지 않는 동안 반복
        ...
        if not visited[node]:  # 방문하지 않은 노드라면
            ...
            for neighbor in graph[node]:  # 인접 노드들을 순회하며 스택에 추가
